...
function startDraft(){
  ...
  if (draftReturnAction === 'modos') {
    const keepH2H=!!matchContext.rematchPending;
    ...
    if(!keepH2H) matchContext.h2h={home:0,away:0};
    matchContext.rematchPending=false;
  }
...
const matchContext = {
  ...
  customHome:null, customAway:null, challengeId:null, h2h:{home:0,away:0}
};
const matchState = {
...
function finishMatch(){
  ...
  if(matchContext.type==='normal'||matchContext.type==='local2p'){
    if(!matchContext.h2h) matchContext.h2h={home:0,away:0};
    if(hs>as) matchContext.h2h.home++; else if(as>hs) matchContext.h2h.away++;
  }
...
function startLocalDraft(){
  ...
  matchContext.h2h={home:0,away:0};
...
function restartLocalDraftForRematch(){
  localDraftState.p1=[];localDraftState.p2=[];localDraftState.turn=0;localDraftState.busy=false;
  document.getElementById('localDraftFormat').textContent=`${localDraftState.total}v${localDraftState.total} · ${localDraftState.deck==='professional'?'PROFISSIONAL':'NORMAL'}`;
  showScreen('localDraftScreen');localDraftRender();localDraftMakeCards();
}
...
function handleSpecialMatchFinish(result){
  ...
  if(matchContext.type==='campaign'){
    ...
    btn.textContent='🔁 REVANCHE (NOVO DRAFT)';btn.dataset.special='local2p-revanche';
    ...
    btn.textContent='🔁 REVANCHE (NOVO DRAFT)';btn.dataset.special='normal-revanche';
  }
  if((matchContext.type==='normal'||matchContext.type==='local2p') && matchContext.h2h && (matchContext.h2h.home+matchContext.h2h.away)>1){
    const note=document.getElementById('finalWinnerNote');
    if(note) note.textContent+=` • Confronto direto: ${matchContext.h2h.home} × ${matchContext.h2h.away}`;
  }
...
function handleSpecialContinue(){
  ...
  if(special==='local2p-revanche'){restartLocalDraftForRematch();return;}
  if(special==='online-revanche'){aceitarRevancheOnline();return;}
  if(special==='normal-revanche'){matchContext.rematchPending=true;draftReturnAction='modos';openDraftSetup('modos');return;}
